...
namespace SnakePort
{
    ...
    public class Game1 : Microsoft.Xna.Framework.Game
    {
        ...
        //Classe que j'ai fait pour les parties du serpent et qui adonnait fonctionner pour la pomme aussi
        Part pomme;
        //classe que jai fait pour le serpent
        Snake python;
        SpriteFont spriteFont;
        //variable pour la chanson qui joue
        Song song;
        //un random pour la place ou je vais faire apparaître
        Random rand = new Random();
        //le moment ou je commence le programme pour pouvoir créer un compteur qui va tempérer la vitesse de jeu
        DateTime start = DateTime.Now;
        //un KeyboardState pour vérifier si la touche viens juste d'être pressée ou si elle est maintenue
        KeyboardState old;
        Texture2D bG;
        // un string pour afficher le score, pas obligatoire... c'était plus un test mais ca fonctionne donc je l'ai laissé la
        string stringScore;
        Vector2 middle;
        Vector2 bGMiddle;
        int score = 0;
        ...
        protected override void LoadContent()
        {
            ...
            //je set des variables que j'ai besoin pendant le jeu je les ai mis dans le LoadContent pour tout pouvoir
            //remettre à zéro en appelant la fonction
        ...
        //une méthode pour générer un endroit aléatoire pour la pomme qui n'est pas directement sur le serpent
        ...
        protected override void Update(GameTime gameTime)
        {
            ...
            foreach (var item in buf)
            {
                ...
                {
                    case Keys.Right:
                        //si la direction est pas le contraire exact (dans snake, on ne peut pas se revirer sur soi-même)
            ...
            if (DateTime.Now >= start.AddMilliseconds(60))
            {
                ...
                //reset le temps
                start = DateTime.Now;
                //textures temporaires a utiliser plus tard
                ...
                //(c'est pout si une pomme a été mangée, le serpent grandit et il ne peut pas magiquement grandir d'un coup)
                ...
                var gameOverChecker = from a in python.parties
                                      ...
                                      select a;
                if (gameOverChecker.Any())
                {
                    //s'il y a correspondance, la partie reset au prochain tour
                    gameOver = true;
                    //et on ne fait pas le reste de la logique car c'est inutile
                    return;
                }
                //sinon on crée la partie à ajouter avec toutes les variables et textures qu'on a besoin
                //qui dépendent de la direction etc.
        ...
        protected override void Draw(GameTime gameTime)
        {
            ...
            //on itère chaque parties du serpent
            for (int i = 0; i < python.parties.Count; i++)
            {
                ...
                //(qui change selon la direction)
                if (i == 0)
                {
                    spriteBatch.Draw(python.parties[i].tailTex, python.parties[i].position, Color.White);
                }
                //si i = denier item de la liste, on a affaire a la tête (la tête c'est tjrs le dernier et la queue en premier) 
                //et on utilise la texture de tête de la partie
                else if (i == python.parties.Count - 1)
                {
                    spriteBatch.Draw(python.parties[i].headTex, python.parties[i].position, Color.White);
                }
                //sinon c'est le corps, donc on draw la texture de corps de la partie
                else
                {
            ...
            //draw le score
            spriteBatch.DrawString(spriteFont, stringScore, Vector2.UnitX, Color.Black);
            spriteBatch.End();
    ...
    class Part
    {
        //position c'est les pixels ou ils vont être à l'écran
